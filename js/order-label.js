window.STF_ORDER_LABEL = (function () {
  ...
  function daceResumidaHtml(order, SENDER) {
    ...
    <div class="label label-dace page-break">
      ...
      <p class="dace-legal">
        ...
        por meio eletrônico (loja oficial Sensor CrashFix). O destinatário é o comprador indicado.
        Documento auxiliar — não substitui Nota Fiscal quando exigida por lei.
      </p>

      <div class="dace-foot">${esc(order.orderId)} · sensorcrashfix.com.br</div>
    </div>`;
  }
  ...
  function footerNote(order) {
    ...
    if (kind === 'motoboy') return `Envio particular · ${esc(service)} · sensorcrashfix.com.br`;
    if (kind === 'uber') return `Uber Direct · sensorcrashfix.com.br`;
    return `Correios · ${esc(service)} · sensorcrashfix.com.br`;
  }
...
<body>
  <div class="label">
    <div class="brand">SENSOR CRASHFIX</div>
