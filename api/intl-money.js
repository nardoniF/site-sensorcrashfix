...
export function syncOpticalIntlBrlFromBrKit(products) {
  // Crash: kit BR (kit completo) e optical-lens-intl (só lente .com) têm preços independentes.
  // Não espelha BRL do kit no produto .com — o Admin define R$ + markup no produto INT.
  return { products: Array.isArray(products) ? products.map((p) => ({ ...p })) : [], synced: false };
}
